\ examples/im2-music/main.fs — entry point.
\
\ AY music driven by an IM 2 ISR, with the rainbow-demo foreground
\ (border colour cycle + random-letter spew) running on top. The ISR
\ writes channel A's 16-bit tone period from a small table once per
...
\
\ Run the resulting .sna in any 128K-capable emulator (Fuse, ZEsarUX).
